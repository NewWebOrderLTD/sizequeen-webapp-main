...
returns trigger as $$
begin
  insert into public.users (id, full_name, avatar_url, gender, age)
  values (new.id, new.raw_user_meta_data->>'full_name', new.raw_user_meta_data->>'avatar_url', new.raw_user_meta_data->>'gender', new.raw_user_meta_data->>'age');
  return new;
end;
...
/**
* USER_MEASUREMENTS
* Note: This table contains user body measurements. Users should only be able to view and update their own data.
*/
...
CREATE TABLE user_measurements (
  ...
  -- Upper Fit
  chest_value DECIMAL(5,2),
  chest_unit VARCHAR(10) DEFAULT 'cm',
  waist_value DECIMAL(5,2),
  waist_unit VARCHAR(10) DEFAULT 'cm',
  arm_value DECIMAL(5,2),
  arm_unit VARCHAR(10) DEFAULT 'cm',
  
  -- Lower Fit
  hip_value DECIMAL(5,2),
  hip_unit VARCHAR(10) DEFAULT 'cm',
  thigh_value DECIMAL(5,2),
  thigh_unit VARCHAR(10) DEFAULT 'cm',
  leg_inseam_value DECIMAL(5,2),
  leg_inseam_unit VARCHAR(10) DEFAULT 'cm',
  
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  
  -- Add unique constraint for user_id to ensure one record per user
  CONSTRAINT unique_user_measurements UNIQUE (user_id)
);

-- Create indexes for better performance
CREATE INDEX idx_user_measurements_user_id ON user_measurements(user_id);
CREATE INDEX idx_user_measurements_created_at ON user_measurements(created_at);
CREATE INDEX idx_user_measurements_updated_at ON user_measurements(updated_at);
...
DROP POLICY IF EXISTS "Users can delete own measurements" ON user_measurements;
...
GRANT ALL ON user_measurements TO authenticated;
GRANT ALL ON user_measurements TO service_role;